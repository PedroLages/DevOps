      ...
      <main className="flex-grow flex flex-col h-[100dvh] bg-white relative min-w-0">
        ...
        <div className="flex-grow overflow-y-auto p-8 lg:p-12 bg-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
            ...
            {/* Empty Slots */}
            <EmptySlot />
            <EmptySlot className="hidden md:flex" />
            <EmptySlot className="hidden xl:flex" />
          </div>
        </div>
      </main>
    </>
...
function EmptySlot({ className = '' }: { className?: string }) {
  return (
    <div className={`bg-slate-50 border border-dashed border-slate-200 rounded-[20px] h-[400px] flex flex-col items-center justify-center text-slate-300 hover:bg-slate-100 hover:text-slate-400 hover:border-slate-300 transition-colors cursor-pointer ${className}`}>
